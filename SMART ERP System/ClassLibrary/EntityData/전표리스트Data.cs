    ...
    public class 전표리스트Data : EntityData<전표리스트>
    {
        ...
        public List<전표리스트> SearchPeriod(DateTime dtpFrom, DateTime dtpTo)
        {
            using (ERPEntities entities = new ERPEntities())
            {
                ...
                DateTime startDay = DateTime.Parse(dtpFrom.ToString("yyyy-MM-dd"));
                DateTime endDay = DateTime.Parse(dtpTo.ToString("yyyy-MM-dd"));

                list = list.Where(x => startDay
                <= DateTime.Parse(x.x.입력날짜.ToString("yyyy-MM-dd")) &&
              DateTime.Parse(x.x.입력날짜.ToString("yyyy-MM-dd")) <= endDay).OrderBy(x => x.x.입력날짜).ToList();
        ...
        public List<전표리스트> Serch손익계산서(DateTime dtpValue, string name, out long left, out long right)
        {
            using (ERPEntities entities = new ERPEntities())
            {
                var query = from x in entities.전표리스트
                            where dtpValue.Date >= x.입력날짜 && x.계정과목.계정과목명.Replace(" ", "") == name.Replace(" ", "")
                            select x;

                var list = query.ToList();

                left = list.Where(x => x.구분 == "차변" || x.구분 == "입금").Sum(x => x.금액);
                right = list.Where(x => x.구분 == "대변" || x.구분 == "출금").Sum(x => x.금액);

                return list;
            }
        }

        public List<전표리스트> Serch합계잔액시산표(DateTime dtpValue, string name, out long left, out long right)
        {
            using (ERPEntities entities = new ERPEntities())
            {
                var query = from x in entities.전표리스트
