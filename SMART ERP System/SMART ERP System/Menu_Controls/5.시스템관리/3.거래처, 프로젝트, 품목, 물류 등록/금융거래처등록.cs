...
namespace SMART_ERP_System.MenuUserControl
{
    public partial class 금융거래처등록 : UserControl
    {
        public 금융거래처등록()
        {
            ...
            var list1 = DB.금융기관.GetAll();
            

        

            var 기관명 = list1.Select(x => x.금융기관명).ToList();
            for (int i = 0; i < 기관명.Count; i++)
            {
                cbb금융기관코드.Items.Add(기관명[i]);
            }
        ...
        private void 금융기관Bds_CurrentChanged(object sender, EventArgs e)
        {
